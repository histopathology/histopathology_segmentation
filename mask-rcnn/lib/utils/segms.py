...
import sys
sys.path.append('/media/hwejin/SSD_1/Code/Github/coco/PythonAPI')


import pycocotools.mask as mask_util


def flip_segms_horizontal(segms, height, width):
...
def flip_segms_vertical(segms, height, width):
  """Up/Down flip each mask in a list of masks."""
  ...
  def _flip_rle(rle, height, width):
    if 'counts' in rle and type(rle['counts']) == list:
      # Magic RLE format handling painfully discovered by looking at the
      # COCO API showAnns function.
      # This sould be modified !!!!
      rle = mask_util.frPyObjects([rle], height, width)
    print ('This should be modified !!!!')
    mask = mask_util.decode(rle)
    mask = mask[:, ::-1, :]
    rle = mask_util.encode(np.array(mask, order='F', dtype=np.uint8))
    return rle
...
'''
def rotate_segms(segms, height, width):
  """Rotate each mask in a list of masks."""

  def _rotate_poly(poly, height):
    flipped_poly = np.array(poly)
    flipped_poly[1::2] = height - np.array(poly[1::2]) - 1
    return flipped_poly.tolist()

  def _rotate_rle(rle, height, width):
    if 'counts' in rle and type(rle['counts']) == list:
      # Magic RLE format handling painfully discovered by looking at the
      # COCO API showAnns function.
      # This sould be modified !!!!
  ...
  for segm in segms:
    if type(segm) == list:
      # Polygon format
      rotated_segms.append([_rotate_poly(poly, height) for poly in segm])
    else:
      # RLE format
      assert type(segm) == dict
      rotated_segms.append(_rotate_rle(segm, height, width))
  return rotated_segms
'''
