...
def combined_roidb_for_training(dataset_names, proposal_files):
    ...
    def get_roidb(dataset_name, proposal_file):
        ...
        #    extend_with_flipped_entries_horizontal(roidb, ds)
        #    logger.info('Appending vertically-flipped training examples...')
        #    extend_with_flipped_entries_vertical(roidb, ds)

            #logger.info('Appending rotated training examples...')
            #extend_with_rotation_entries(roidb, ds)
...
def extend_with_flipped_entries_horizontal(roidb, dataset):
    ...
    for entry in roidb:
        ...
        flipped_entry['segms'] = segm_utils.flip_segms_horizontal(
...
def extend_with_flipped_entries_vertical(roidb, dataset):
    #########
    """Flip each entry in the given roidb and return a new roidb that is the
    concatenation of the original roidb and the flipped entries.
    ...
    roidb.extend(flipped_roidb)
'''
def extend_with_rotation_entries(roidb, dataset):
    #########
    """Rotate each entry in the given roidb and return a new roidb that is the
    concatenation of the original roidb and the rotated entries.

    "Rotating" an entry means that that image and associated metadata (e.g.,
    ground truth boxes and object proposals) are rotated.
    """
    rotated_roidb = []
    for entry in roidb:
        width = entry['width']
        ...
        for i in range(3):
            if i == 0:
                boxes[:, 0] = oldy1 - 1
                boxes[:, 2] = width - oldx2 - 1
                boxes[:, 1] = oldy2 - 1
                boxes[:, 3] = width - oldx1 - 1
            elif i == 1:
                boxes[:, 0] = width - oldx2 - 1
                boxes[:, 2] = height - oldy2 - 1
                boxes[:, 1] = width - oldx1 - 1
                boxes[:, 3] = height - oldy1 - 1
            else:
                boxes[:, 0] = height - oldy2 - 1
                boxes[:, 2] = oldx1 - 1
                boxes[:, 1] = height - oldy1 - 1
                boxes[:, 3] = oldx2 - 1
            ...
            for k, v in entry.items():
                if k not in dont_copy:
                    rotated_entry[k] = v
            rotated_entry['boxes'] = boxes
            rotated_entry['segms'] = segm_utils.rotate_segms(
                entry['segms'], entry['height'], entry['width']
            )
            if dataset.keypoints is not None:
                ###todo
                print ('todo')
                continue
            rotated_entry['rotated'] = True
            rotated_roidb.append(rotated_entry)
    roidb.extend(rotated_roidb)
'''
