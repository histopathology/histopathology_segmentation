...
class SampleProvider(object):
  ...
  def _transform(self, images_org):
    ...
    if self.image_options["crop"]:
        ...
        if self.image_options["resize_size"] == images_org.shape[0]:
            image = images_org
        else:
            resize_size = int(self.image_options["resize_size"])
            y  = np.random.permutation(range(int(resize_size/2), images_org.shape[0]-int(resize_size/2)))
            y = int(y[0])
            y1 = int(y - resize_size/2.0)
            y2 = int(y + resize_size/2.0)
            
            x  = np.random.permutation(range(int(resize_size/2), images_org.shape[1]-int(resize_size/2)))
            x = int(x[0])
